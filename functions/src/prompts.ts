import type { ProfilerInput } from "./schemas.js";

/** Versión del prompt bundle (auditoría / comparación de informes). */
export const PROMPT_BUNDLE_VERSION = "4";

function serializeRecord(label: string, obj: Record<string, string>): string {
  return `${label}:\n${Object.entries(obj)
    .map(([k, v]) => `  - ${k}: ${v}`)
    .join("\n")}`;
}

export function buildProfilerContext(input: ProfilerInput): string {
  ...
  if ("schemaVersion" in answers && answers.schemaVersion === 2) {
    ...
    if (a.userType === "company") {
      ...
      if (a.companyPath === "automation") {
        base.push(serializeRecord("Automation flow", a.automation as unknown as Record<string, string>));
      } else if (a.companyPath === "new_product") {
        base.push(serializeRecord("New product flow", a.newProduct as unknown as Record<string, string>));
      } else {
        base.push(serializeRecord("Unsure / exploratory flow", a.unsure as unknown as Record<string, string>));
      }
    } else if (a.userType === "startup") {
      base.push(serializeRecord("Startup flow", a.startup as unknown as Record<string, string>));
    } else {
      base.push(serializeRecord("Freelancer flow", a.freelancer as unknown as Record<string, string>));
    }
    return base.join("\n");
  }

  const leg = answers as {
    userType: string;
    companyBranch?: string;
    startupStage?: string;
    freelancerFocus?: string;
...
export const SYSTEM_BUNDLE = `You are a senior digital transformation consultant for Aurora (AI-driven software factory).
