...
export async function sendDiagnosticEmail(opts: {
  ...
  const diagnosticMeetingUrl =
    process.env.DIAGNOSTIC_MEETING_URL ??
  ...
  const htmlEs = buildBrandedHtml({
    ...
    intro:
      "Ya preparamos tu diagnóstico en base a lo que nos compartiste. No reemplaza una reunión, pero te da una visión clara de por dónde empezar y qué evitar.",
    ...
    nextStepIntro:
      "Si quieres llevar esto a algo real, te recomiendo agendar una llamada con nosotros.",
    nextStepItems: [
  ...
  const htmlEn = buildBrandedHtml({
    ...
    intro:
      "We prepared your diagnostic based on what you shared. It does not replace a meeting, but it gives you a clear view of where to start and what to avoid.",
    ...
    nextStepIntro:
      "If you want to turn this into something real, I recommend booking a call with us.",
    nextStepItems: [
    ...
    plainReportUrl: escapeHtml(plainUrlForEmail),
    replyLine:
      "If you have specific questions, reply to this email and we will review them with you.",
